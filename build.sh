...
do
    ...
    then
        ...
        printf "%b %bINFO:%b  ." "$(date "+%Y-%m-%d %H:%M:%b")" "${fmt_bold}" "${fmt_end}"
    else
        printf "."
    fi
    i=$(( i + 1 ))
...
if [ ! -f install"${openbsd_version_short}".img ]; then
    ...
    then
        ...
        printf "%b %bERROR:%b https://cdn.openbsd.org/pub/OpenBSD/%b/arm64/install%b.img\n" "$(date "+%Y-%m-%d %H:%M:%b")" "${fmt_red_bold}" "${fmt_end}" "${openbsd_version_log}" "${openbsd_version_short}"
        exit 11
    fi
fi
